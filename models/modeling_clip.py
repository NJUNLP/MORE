...
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.depth_embedding = nn.Conv2d(
            in_channels=1, out_channels=self.embed_dim, kernel_size=self.patch_size, stride=self.patch_size, bias=False)
    ...
    def forward(self, pixel_values):
        ...
        if pixel_values.shape[-3] == 4:
            depth_values = pixel_values[:, -1:, :, :]
            pixel_values = pixel_values[:, :-1, :, :]
            depth_embeds = self.depth_embedding(depth_values)
            patch_embeds = self.patch_embedding(pixel_values)  # shape = [*, width, grid, grid]
            patch_embeds = patch_embeds.flatten(2).transpose(1, 2)
            depth_embeds = depth_embeds.flatten(2).transpose(1, 2)
            patch_embeds = patch_embeds + self.position_embedding(self.position_ids)
            depth_embeds = depth_embeds + self.position_embedding(self.position_ids)
            return patch_embeds, depth_embeds
        else:
            patch_embeds = self.patch_embedding(pixel_values)  # shape = [*, width, grid, grid]
            patch_embeds = patch_embeds.flatten(2).transpose(1, 2)
            patch_embeds = patch_embeds + self.position_embedding(self.position_ids)
